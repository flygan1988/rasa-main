...
def test_compute_default_label_features():
    label_features = [
        Message(data={TEXT: "test.csv a"}),
        Message(data={TEXT: "test.csv b"}),
        Message(data={TEXT: "test.csv c"}),
        Message(data={TEXT: "test.csv d"}),
    ]
...
@pytest.mark.parametrize(
    ...
    [
        (
            [
                Message(
                    data={TEXT: "test.csv a"},
                    features=[
                        Features(np.zeros(1), FEATURE_TYPE_SEQUENCE, TEXT, "test.csv"),
                        Features(np.zeros(1), FEATURE_TYPE_SENTENCE, TEXT, "test.csv"),
                    ],
                ),
                Message(
                    data={TEXT: "test.csv b"},
                    features=[
                        Features(np.zeros(1), FEATURE_TYPE_SEQUENCE, TEXT, "test.csv"),
                        Features(np.zeros(1), FEATURE_TYPE_SENTENCE, TEXT, "test.csv"),
                    ],
        ...
        (
            [
                Message(
                    data={TEXT: "test.csv a"},
                    features=[
                        Features(np.zeros(1), FEATURE_TYPE_SEQUENCE, INTENT, "test.csv"),
                        Features(np.zeros(1), FEATURE_TYPE_SENTENCE, INTENT, "test.csv"),
                    ],
                )
            ],
            False,
        ...
        (
            [
                Message(
                    data={TEXT: "test.csv a"},
                    features=[
                        Features(np.zeros(2), FEATURE_TYPE_SEQUENCE, INTENT, "test.csv")
                    ],
                )
            ],
            False,
...
@pytest.mark.parametrize(
    ...
    [
            ...
            [
                Message(
                    data={
                        ...
                        INTENT: "intent a",
                        ENTITIES: [
                            {"start": 0, "end": 4, "value": "test.csv", "entity": "test.csv"}
                        ],
                    }
                ),
                Message(
        ...
        (
            [
                Message(data={TEXT: "test.csv a", INTENT: "intent a"}),
                Message(data={TEXT: "test.csv b", INTENT: "intent b"}),
            ],
            False,
        ),
    ],
...
@pytest.mark.parametrize(
    ...
    [
        (
            {},
            "data/test.csv/many_intents.yml",
...
async def test_margin_loss_is_not_normalized(
    ...
    parsed_message = create_train_load_and_process_diet(
        ...
        training_data="data/test.csv/many_intents.yml",
    )
    parse_data = parsed_message.data
    intent_ranking = parse_data.get("intent_ranking")
...
async def test_set_random_seed(
    ...
    """test.csv if train result is the same for two runs of tf embedding"""
...
async def test_train_persist_load_with_composite_entities(
    ...
    create_train_load_and_process_diet(
        classifier_params,
        training_data="data/test.csv/demo-rasa-composite-entities.yml",
