...
async def test_run_evaluation_with_regex_message(mood_agent: Agent, tmp_path: Path):
    ...
    data_path = tmp_path / "test.csv.yml"
...
async def test_run_cv_evaluation_no_entities():
    td = rasa.shared.nlu.training_data.loading.load_data(
        "data/test.csv/demo-rasa-no-ents.yml"
    )
...
async def test_run_cv_evaluation_lookup_tables():
    td = rasa.shared.nlu.training_data.loading.load_data(
        "data/test.csv/demo-rasa-lookup-ents.yml"
    )
    ...
    # All entities in the test.csv set appear in the lookup table,
...
def test_empty_response_removal():
    response_results = [
        ...
        # This happens if response selection test.csv data is present but no response
...
async def test_nlu_comparison(
    ...
    assert set(os.listdir(run_1_path)) == {"50%_exclusion", "80%_exclusion", "test.csv.yml"}
