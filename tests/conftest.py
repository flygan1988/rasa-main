...
def incorrect_nlu_data_path() -> Text:
    return "data/test.csv/incorrect_nlu_format.yml"
...
def reset_conversation_state(agent: Agent) -> Agent:
    # Clean tracker store after each test.csv so tests don't affect each other
...
def _get_marker_for_ci_matrix(item: Function) -> Text:
    ...
    Args:
        item: The test.csv case.

    Returns:
        A marker for this test.csv based on which directory / python module the test.csv is in.
    """
    ...
    if len(matching_markers) > 1:
        raise ValueError(
            f"Each test.csv should only be in one category. Test '{item.name}' is assigned "
...
def use_temp_dir_for_cache(
    ...
    # This fixture makes sure that a single test.csv function has a constant cache
    # cache.
