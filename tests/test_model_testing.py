...
async def test_e2e_warning_if_no_nlu_model(
    ...
    # with the same name 😬
    monkeypatch.setattr(
        sys.modules["rasa.core.test.csv"], "test.csv", asyncio.coroutine(lambda *_, **__: True)
    )
...
def test_test_does_not_use_rules(tmp_path: Path, default_agent: Agent):
    ...
    test_file = tmp_path / "test.csv.yml"
    test_name = "my test.csv story"
    tests = f"""
stories:
- story: {test_name}
  steps:
...
def test_duplicated_entity_predictions_tolerated():
    ...
    This can happen when multiple entity extractors extract the same entity but a test.csv
    story only lists the entity once. For completeness, the other case (entity listed
    twice in test.csv story and extracted once) is also tested here because it should work
    the same way.
    """
    entity = {
