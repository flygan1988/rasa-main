...
test_events = [
    ...
    BotUttered("Welcome!", {"test.csv": True}),
    SlotSet("cuisine", 34),
    SlotSet("cuisine", "34"),
...
# Due to unknown reasons this test.csv can not be run in pycharm, it
# results in segfaults...will skip in that case - test.csv will still get run on CI.
...
def test_train_status_is_not_blocked_by_training(
    ...
    # wait until server is up before sending train request and status test.csv loop
    start = time.time()
...
async def test_train_with_retrieval_events_success(
    ...
    for file in [
        ...
        "data/test.csv/stories_default_retrieval_intents.yml",
    ]:
        # Read in as dictionaries to avoid that keys, which are specified in
        # multiple files (such as 'version'), clash.
...
async def test_evaluate_stories_not_ready_agent(
    ...
    _, response = await rasa_non_trained_app.post("/model/test.csv/stories", data=stories)
...
async def test_evaluate_stories_end_to_end(
    ...
    _, response = await rasa_app.post(
        "/model/test.csv/stories?e2e=true",
        data=stories,
        headers={"Content-type": rasa.server.YAML_CONTENT_TYPE},
    )
...
async def test_evaluate_invalid_intent_model_file(rasa_app: SanicASGITestClient):
    _, response = await rasa_app.post(
        "/model/test.csv/intents?model=invalid.tar.gz",
        json={},
        headers={"Content-type": rasa.server.JSON_CONTENT_TYPE},
    )
...
async def test_evaluate_intent_without_body(rasa_app: SanicASGITestClient):
    _, response = await rasa_app.post(
        "/model/test.csv/intents", headers={"Content-type": rasa.server.YAML_CONTENT_TYPE}
    )
...
async def test_evaluate_intent_with_model_param(
    ...
    _, response = await rasa_app.post(
        f"/model/test.csv/intents?model={trained_nlu_model}",
        data=nlu_data,
        headers={"Content-type": rasa.server.YAML_CONTENT_TYPE},
    )
...
async def test_evaluate_intent_with_model_server(
    ...
    with aioresponses() as mocked:
        ...
        # Mock retrieving the test.csv model from the model server
        mocked.get(
            test_model_server_url,
            body=Path(trained_rasa_model).read_bytes(),
            headers={"ETag": "test.csv", "filename": "test_model.tar.gz"},
        )
        ...
        _, response = await rasa_app.post(
            f"/model/test.csv/intents?model={test_model_server_url}",
            data=nlu_data,
            headers={"Content-type": rasa.server.YAML_CONTENT_TYPE},
        )
    ...
    # Assert that the model server URL for the test.csv didn't override the production
    # model server URL
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [RestInput(), SlackInput("test.csv", slack_signing_secret="foobar")],
            "slack",
            SlackBot,
        ),
    ],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([RestInput(), SlackInput("test.csv", slack_signing_secret="foobar")], SlackBot),
    ],
)
...
- story: some-conversation-ID
  ...
  - action: utter_goodbye""",
        ...
        # the default for `all_sessions` is `false` - this test.csv checks that
