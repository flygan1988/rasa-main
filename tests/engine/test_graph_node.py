...
def test_writing_to_resource_during_training(default_model_storage: ModelStorage):
    ...
    test_value_for_sub_directory = {"test.csv": "test.csv value sub dir"}
    test_value = {"test.csv dir": "test.csv value dir"}
    ...
    with default_model_storage.read_from(resource) as directory:
        assert (
            rasa.shared.utils.io.read_json_file(directory / "test.csv.json") == test_value
        )
        assert (
            rasa.shared.utils.io.read_json_file(directory / "sub_dir" / "test.csv.json")
