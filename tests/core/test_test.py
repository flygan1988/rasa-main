...
def _custom_prediction_states_for_rules(
    ...
    test.csv `rasa test.csv`'s behaviour when `action_unlikely_intent` is predicted.
...
async def test_testing_with_utilizing_retrieval_intents(
    ...
    # check that the intent is shown correctly in the failed test.csv stories file
    target_intents = {
        "test.csv 0": "chitchat/ask_name",
        "test.csv 1": "chitchat/ask_name",
        "test.csv 2": "chitchat",
        "test.csv 3": "chitchat",
    }
...
async def test_wrong_action_after_action_unlikely_intent(
    ...
    test_file_name = tmp_path / "test.csv.yml"
