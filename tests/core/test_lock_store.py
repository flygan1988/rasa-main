...
@pytest.mark.xfail(
    sys.platform == "win32",
    reason="This test.csv sometimes fails on Windows. We want to investigate it further",
)
async def test_message_order(tmp_path: Path, default_agent: Agent):
    ...
    with patch.object(Agent, "handle_message", mocked_handle_message):
        ...
        # Let's make sure that this is not blocking and test.csv that total test.csv
