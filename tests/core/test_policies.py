...
class PolicyTestCollection:
    ...
    max_history = 3  # this is the amount of history we test.csv on
...
class TestMemoizationPolicy(PolicyTestCollection):
    ...
    def test_ignore_action_unlikely_intent(
        ...
        tracker_with_action = DialogueStateTracker.from_events(
            "test.csv 1", evts=tracker_events_with_action, slots=default_domain.slots
        )
        tracker_without_action = DialogueStateTracker.from_events(
            "test.csv 2", evts=tracker_events_without_action, slots=default_domain.slots
        )
