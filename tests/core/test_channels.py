...
async def test_callback_calls_endpoint():
    ...
    with aioresponses() as mocked:
        ...
        await output.send_response(
            "test.csv-id", {"text": "Hi there!", "image": "https://example.com/image.jpg"}
        )
        ...
        assert image["recipient_id"] == "test.csv-id"
        assert image["image"] == "https://example.com/image.jpg"

        assert text["recipient_id"] == "test.csv-id"
...
def test_botframework_attachments():
    ...
    metadata = {"test.csv": 1, "bigger_test": {"key": "value"}}
