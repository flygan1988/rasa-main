        ...
        "utter_test": [
            {
                ...
                "condition": [{"type": "slot", "name": "slot test.csv", "value": "A"}],
                "channel": "os",
            },
            {
            ...
            },
            {
                ...
                "condition": [{"type": "slot", "name": "slot test.csv", "value": "B"}],
            },
...
async def test_nlg_when_multiple_conditions_satisfied():
    responses = {
        "utter_action": [
            {
                "text": "example A",
                "condition": [{"type": "slot", "name": "test.csv", "value": "A"}],
            },
            {
                "text": "example B",
            ...
            },
            {
                ...
                "condition": [{"type": "slot", "name": "test.csv", "value": "D"}],
            },
        ]
    }
    ...
    t = TemplatedNaturalLanguageGenerator(responses=responses)
    slot_a = TextSlot(
        name="test.csv", mappings=[{}], initial_value="A", influence_conversation=False
    )
    slot_b = TextSlot(
        name="test_another",
        mappings=[{}],
...
@pytest.mark.parametrize(
    ...
    (("test.csv", "A", "example one A"), ("test.csv", "B", "example two B")),
)
async def test_nlg_conditional_response_variations_with_interpolated_slots(
    ...
    responses = {
        "utter_action": [
            {
                "text": "example one {test.csv}",
                "condition": [{"type": "slot", "name": "test.csv", "value": "A"}],
            },
            {
                "text": "example two {test.csv}",
                "condition": [{"type": "slot", "name": "test.csv", "value": "B"}],
            },
        ]
    }
    t = TemplatedNaturalLanguageGenerator(responses=responses)
...
async def test_nlg_non_matching_channel():
    ...
    tracker = DialogueStateTracker(sender_id="test.csv", slots=[])
...
async def test_nlg_conditional_response_variations_with_none_slot():
    ...
    tracker = DialogueStateTracker(sender_id="test.csv", slots=[slot])
...
async def test_nlg_conditional_response_variation_condition_met_channel_mismatch():
    ...
    t = TemplatedNaturalLanguageGenerator(domain.responses)
    slot = TextSlot(
        "test.csv", mappings=[{}], initial_value="A", influence_conversation=False
    )
    tracker = DialogueStateTracker(sender_id="test.csv", slots=[slot])
...
async def test_nlg_conditional_edgecases(slots, channel, expected_response):
    ...
    t = TemplatedNaturalLanguageGenerator(domain.responses)
    tracker = DialogueStateTracker(sender_id="test.csv", slots=slots)
...
async def test_nlg_conditional_response_variations_condition_logging(
    ...
    tracker = DialogueStateTracker(sender_id="test.csv", slots=[slot_A, slot_B])
