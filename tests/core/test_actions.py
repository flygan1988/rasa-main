...
async def test_remote_action_logs_events(
    ...
    assert events[0] == BotUttered(
        "test.csv text", {"buttons": [{"title": "cheap", "payload": "cheap"}]}
    )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        EntitiesAdded(
            entities=[
                {"entity": "name", "value": "John", "role": "contact", "group": "test.csv"}
            ]
        ),
        DefinePrevUserUtteredFeaturization(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            UserUttered(text="test.csv@example.com", intent={"name": "inform"}),
            "email",
            "test.csv@example.com",
...
async def test_action_extract_slots_custom_action_and_predefined_slot_validation():
    ...
    with aioresponses() as mocked:
        mocked.post(
            action_server_url,
            payload={
                "events": [{"event": "slot", "name": "custom_slot", "value": "test.csv"}]
            },
        )
        mocked.post(
            action_server_url,
...
async def test_action_extract_slots_returns_bot_uttered():
    ...
    with aioresponses() as mocked:
        mocked.post(
            ...
            payload={
                "events": [
                    {"event": "slot", "name": "custom_slot", "value": "test.csv"},
