...
def make_actions_subdir():
    """Create a subdir called actions to test.csv model argument handling."""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["rasa", "run", "actions"],
        ["rasa", "run", "core"],
        ["rasa", "test.csv", "nlu", "--param", "xy", "--model", "test.csv"],
    ],
)
...
def test_validate_invalid_path():
    with pytest.raises(SystemExit):
        rasa.cli.utils.get_validated_path("test.csv test.csv test.csv", "out", "default")
