...
def test_lookup_table_json():
    lookup_fname = "data/test.csv/lookup_tables/plates.txt"
    td_lookup = load_data("data/test.csv/lookup_tables/lookup_table.json")
...
def test_lookup_table_yaml():
    td_lookup = load_data("data/test.csv/lookup_tables/lookup_table.yml")
    assert not td_lookup.is_empty()
    assert len(td_lookup.lookup_tables) == 1
    assert td_lookup.lookup_tables[0]["name"] == "plates"
...
def test_composite_entities_data():
    td = load_data("data/test.csv/demo-rasa-composite-entities.yml")
    assert not td.is_empty()
...
@pytest.mark.parametrize(
    "files",
    [
        ("data/examples/rasa/demo-rasa.json", "data/test.csv/multiple_files_json"),
        ("data/examples/rasa/demo-rasa.yml", "data/test.csv/duplicate_intents_yaml"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "data/test.csv/wit_converted_to_rasa.json",
            "json",
            None,
        ),
        (
            "data/examples/luis/demo-restaurants_v7.json",
            "data/test.csv/luis_converted_to_rasa.json",
            "json",
            None,
        ),
        (
            "data/examples/dialogflow/",
            "data/test.csv/dialogflow_en_converted_to_rasa.json",
            "json",
            "en",
        ),
        (
            "data/examples/dialogflow/",
            "data/test.csv/dialogflow_es_converted_to_rasa.json",
            "json",
            "es",
        ),
        (
            "data/examples/rasa/demo-rasa.yml",
            "data/test.csv/md_converted_to_json.json",
            "json",
            None,
        ),
    ],
...
def test_without_additional_e2e_examples(tmp_path: Path):
    ...
    # Patch to return our test.csv stories
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"name": "plates", "elements": "data/test.csv/lookup_tables/plates.txt"},
            {
                "name": "plates",
                "elements": "tacos\nbeef\nmapo tofu\nburrito\nlettuce wrap",
            },
        ...
        (
            {
                "name": "plates",
                "elements": "data/test.csv/lookup_tables/not-existing-file.txt",
            },
            {
                "name": "plates",
                "elements": "data/test.csv/lookup_tables/not-existing-file.txt",
        ...
        (
            {"name": "test.csv", "some_key": "some_value", "elements": "everything else"},
            {"name": "test.csv", "some_key": "some_value", "elements": "everything else"},
        ),
    ],
)
...
def test_fingerprint_is_different_when_lookup_table_has_changed(
    ...
    files = ["data/test.csv/lookup_tables/lookup_table.json"]
