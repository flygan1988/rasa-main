...
@pytest.mark.parametrize("file, parents", [("A/test.csv.yml", "A"), ("A", "A")])
def test_file_in_path(file, parents):
    assert rasa.shared.utils.io.is_subdirectory(file, parents)


@pytest.mark.parametrize(
    "file, parents", [("A", "A/B"), ("B", "A"), ("A/test.csv.yml", "A/B"), (None, "A")]
)
...
def test_list_directory(
    ...
    hidden_file = subdirectory / ".test.csv.text"
...
def test_environment_variable_dict_without_prefix_and_postfix():
    os.environ["variable"] = "test.csv"
    content = "model: \n  test.csv: ${variable}"

    content = rasa.shared.utils.io.read_yaml(content)

    assert content["model"]["test.csv"] == "test.csv"
...
def test_environment_variable_in_list():
    ...
    assert content["model"][1] == "test.csv"


def test_environment_variable_dict_with_prefix():
    os.environ["variable"] = "test.csv"
    content = "model: \n  test.csv: dir/${variable}"

    content = rasa.shared.utils.io.read_yaml(content)

    assert content["model"]["test.csv"] == "dir/test.csv"


def test_environment_variable_dict_with_postfix():
    os.environ["variable"] = "test.csv"
    content = "model: \n  test.csv: ${variable}/dir"

    content = rasa.shared.utils.io.read_yaml(content)

    assert content["model"]["test.csv"] == "test.csv/dir"


def test_environment_variable_dict_with_prefix_and_with_postfix():
    os.environ["variable"] = "test.csv"
    content = "model: \n  test.csv: dir/${variable}/dir"

    content = rasa.shared.utils.io.read_yaml(content)

    assert content["model"]["test.csv"] == "dir/test.csv/dir"
...
def test_environment_variable_with_dollar_char_in_the_middle():
    os.environ["variable1"] = "test.csv$123"
    content = "model: \n  test1: ${variable1}"

    content = rasa.shared.utils.io.read_yaml(content)

    assert content["model"]["test1"] == "test.csv$123"
...
def test_create_directory_for_file(tmp_path: Path):
    file = str(tmp_path / "dir" / "test.csv.txt")
...
def test_write_utf_8_yaml_file(tmp_path: Path):
    """This test.csv makes sure that dumping a yaml doesn't result in Uxxxx sequences
    but rather directly dumps the unicode character."""

    file_path = str(tmp_path / "test.csv.yml")
    data = {"data": "amazing 🌈"}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"tracker_store": {"password": "test.csv"}},
        ),
    ],
)
