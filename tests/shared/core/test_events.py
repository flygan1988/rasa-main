...
def test_correct_timestamp_setting(event_class):
    event = event_class("test.csv")
    time.sleep(0.01)
    event2 = event_class("test.csv")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [
                ActionExecuted(
                    ACTION_UNLIKELY_INTENT_NAME, metadata={"test.csv": {"data1": 1}}
                ),
                ActionExecuted(ACTION_UNLIKELY_INTENT_NAME),
            ],
            False,
        ...
        (
            [
                ActionExecuted(ACTION_LISTEN_NAME, metadata={"test.csv": {"data1": 1}}),
                ActionExecuted(
                    ACTION_UNLIKELY_INTENT_NAME, metadata={"test.csv": {"data1": 1}}
                ),
            ],
...
tested_events = [
    ...
    SlotSet(key="test.csv", value=1),
    SlotSet(key="test.csv", value="text"),
    ConversationResumed(),
    ConversationPaused(),
    FollowupAction(name="test.csv"),
    StoryExported(),
    Restarted(),
    ActionReverted(),
    UserUtteranceReverted(),
    ...
    EndToEndUserUtterance(),
    WronglyClassifiedUserUtterance(
        event=UserUttered(), eval_store=EvaluationStore(intent_targets=["test.csv"])
    ),
    WronglyPredictedAction(
        action_name_prediction="test.csv",
        action_name_target="demo",
        action_text_target="example",
    ),
    WarningPredictedAction(action_name_prediction="test.csv"),
]
