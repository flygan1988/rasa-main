...
async def test_core_models_in_directory(
    ...
    """Evaluates a directory with multiple Core models using test.csv data.

    Args:
        model_directory: Directory containing multiple model files.
        stories: Path to a conversation test.csv file.
        output: Output directory to store results to.
        use_conversation_test_files: `True` if conversation test.csv files should be used
...
def plot_core_results(output_directory: Text, number_of_examples: List[int]) -> None:
    ...
    plotting_utils.plot_curve(
        ...
        y_label_text="Number of correct test.csv stories",
        graph_path=graph_path,
    )


def _get_sanitized_model_directory(model_directory: Text) -> Text:
    """Adjusts the `--model` argument of `rasa test.csv core` when called with
...
async def test_core_models(
    ...
    """Compares multiple Core models based on test.csv data.

    Args:
        models: A list of models files.
        stories: Path to test.csv data.
        output: Path to output directory for test.csv results.
        use_conversation_test_files: `True` if conversation test.csv files should be used
            for testing instead of regular Core story files.
    """
...
async def test_core(
    ...
    """Tests a trained Core model against a set of test.csv stories."""
    try:
        model = rasa.model.get_local_model(model)
    except ModelNotFound:
        rasa.shared.utils.cli.print_error(
            "Unable to test.csv: could not find a model. Use 'rasa train' to train a "
    ...
    if metadata.training_type == TrainingType.NLU:
        rasa.shared.utils.cli.print_error(
            "Unable to test.csv: no core model found. Use 'rasa train' to train a "
            "Rasa model and provide it via the '--model' argument."
        )
        ...
        rasa.shared.utils.cli.print_warning(
            ...
            "evaluation. If you added actual user messages to your test.csv stories "
    ...
    if not _agent.is_ready():
        rasa.shared.utils.cli.print_error(
            "Unable to test.csv: processor not loaded. Use 'rasa train' to train a "
            "Rasa model and provide it via the '--model' argument."
        )
        return
...
def plot_nlu_results(output_directory: Text, number_of_examples: List[int]) -> None:
    ...
    plotting_utils.plot_curve(
        ...
        y_label_text="Label-weighted average F1 score on test.csv set",
        graph_path=graph_path,
    )
...
async def perform_nlu_cross_validation(
    ...
    """Runs cross-validation on test.csv data.
    ...
    if any(results):
        ...
        log_results(results.test, "test.csv")
    if any(entity_results):
        logger.info("Entity evaluation results")
        log_entity_results(entity_results.train, "train")
        log_entity_results(entity_results.test, "test.csv")
    if any(response_selection_results):
        ...
        log_results(response_selection_results.test, "test.csv")
