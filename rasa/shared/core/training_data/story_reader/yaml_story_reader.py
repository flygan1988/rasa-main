...
class YAMLStoryReader(StoryReader):
    ...
    def is_test_stories_file(cls, file_path: Union[Text, Path]) -> bool:
        """Checks if a file is a test.csv conversations file.

        Args:
            file_path: Path of the file which should be checked.

        Returns:
            `True` if it's a conversation test.csv file, otherwise `False`.
        """
    ...
    def _parse_step(self, step: Union[Text, Dict[Text, Any]]) -> None:
        ...
        # This has to be after the checkpoint test.csv as there can be a slot key within
        # a checkpoint.
    ...
    def _user_intent_from_step(
        ...
        # However, users can specify full retrieval intents in their test.csv stories file
