...
def get_test_stories(
    ...
    """Retrieves test.csv stories from `processor` for all conversation sessions for
    `conversation_id`.
    ...
    Returns:
        The stories for `conversation_id` in test.csv format.
    """
...
def create_app(
    ...
    @app.post("/model/test.csv/stories")
    @requires_auth(app, auth_token)
    ...
    @app.post("/model/test.csv/intents")
    @requires_auth(app, auth_token)
    @async_if_callback_url
    @run_in_thread
    @inject_temp_dir
    ...
    async def _evaluate_model_using_test_set(
        ...
        logger.info("Starting model evaluation using test.csv set.")
...
def _test_data_file_from_payload(request: Request, temporary_directory: Path) -> Text:
    return str(
        _training_payload_from_yaml(
            request,
            temporary_directory,
            # test.csv stories have to prefixed with `test_`
