...
def _collect_user_uttered_predictions(
    ...
    # intent from the test.csv story, may either be base intent or full retrieval intent
    ...
    # if the test.csv story only provides the base intent AND the prediction was correct,
...
def emulate_loop_rejection(partial_tracker: DialogueStateTracker) -> None:
    ...
    test.csv unhappy paths of the loops, we need to emulate loop rejection.
...
def _get_predicted_action_name(
    ...
    a test.csv story is a retrieval action but it's not specified in the test.csv story.
...
async def _predict_tracker_actions(
    ...
    for event in events[1:]:
        if isinstance(event, ActionExecuted):
            ...
            # Indirectly that means that the test.csv story was in YAML format.
            if not event.text:
                predicted = event.parse_data
            # Indirectly that means that the test.csv story was either:
...
async def compare_models_in_dir(
    ...
    """Evaluates multiple trained models in a directory on a test.csv set.
...
async def compare_models(
    ...
    """Evaluates multiple trained models on a test.csv set.
