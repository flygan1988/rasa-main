...
async def get_eval_data(
    ...
    """Runs the model for the test.csv set and extracts targets and predictions.
...
async def run_evaluation(
    ...
    Args:
        data_path: path to the test.csv data
...
async def cross_validate(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if intent_test_results:
            logger.info("Accumulated test.csv folds intent evaluation results:")
        ...
        if entity_test_results:
            logger.info("Accumulated test.csv folds entity evaluation results:")
        ...
        if response_selection_test_results:
            logger.info("Accumulated test.csv folds response selection evaluation results:")
...
async def compare_nlu(
    ...
    Afterwards, the model is tested on the complete test.csv data of that run.
...
def log_results(results: IntentMetrics, dataset_name: Text) -> None:
    ...
    Args:
        ...
        dataset_name: string of which dataset the results are from, e.g. test.csv/train
    """
